...
def tuo_bang_aws():
    ...
    if uploaded_file is not None:
        ...
        df = df[df['lineItem/UsageAccountId'] == 904213552531]
        st.write("904213552531数据")
        st.write(df.shape[0])
        st.dataframe(df)


        #product/pricingUnit列筛选：Hrs
        df_hrs = df[df['pricing/unit'] == 'Hrs']
        st.write("Hrs数据")
        ...
        st.dataframe(new_df)
        #过滤的
        #        new_df = new_df[new_df['lineItem/LineItemDescription'].str.contains('reserved') & new_df['pricing/term'].str.contains('Reserved')]
        ...
        # #lineItem/ProductCode列分别筛选：AmazonRDS、AmazonEC2
        # df_rds = df[df['lineItem/ProductCode'] == 'AmazonRDS'] 
        # st.write("RDS数据")
        # st.dataframe(df_rds)
      

        # df_ec2 = df[df['lineItem/ProductCode'] == 'AmazonEC2']
        # st.write("EC2数据")
        # st.dataframe(df_ec2)
